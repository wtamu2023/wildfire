# Generate map of New Mexico and surrounding states, wildfire points, 
...
txData <- map_data("state", region=c('Texas'))
okData <- map_data("state", region=c('Oklahoma'))
...
ggplot() + 
  ...
  geom_polygon(data=okData, aes(x=long, y=lat),
               color="#000000", fill='#f1f1f1') +
  geom_polygon(data=txData, aes(x=long, y=lat),
               color="#000000", fill='#f1f1f1') +
  ...
  geom_polygon(data=coData, aes(x=long, y=lat),
               color="#000000", fill='#f1f1f1') +
  coord_cartesian(xlim =c(-110, -102.5), ylim = c(31, 38))+
  ...
  labs(title="Wildfires in New Mexico and Surrounding US States (2000 to 2022)",
       subtitle= 'Cities with Population of over 200,000 Labeled with Name',
